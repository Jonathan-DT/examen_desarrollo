...
class Pacientes
{
    ...
    public function get_data()
    {
        ...
        if ($consulta = $this->db->query($sql)) {
            ...
            $this->paciente = array(); // Inicializa el array vacío

            while ($filas = $consulta->fetch_assoc()) {
                $this->paciente[] = $filas; // Agrega cada fila al array $paciente
            }
    ...
    {
        // Preparar la consulta SQL
        $sql = "INSERT INTO pacientes (nombre,curp) VALUES (?, ?)";

        // Preparar la sentencia
        if ($stmt = $this->db->prepare($sql)) {
            // Enlazar los parámetros
            $stmt->bind_param('ss', $name, $curp);
